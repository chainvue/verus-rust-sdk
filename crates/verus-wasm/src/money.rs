...
pub fn parse_coins(coins: &str) -> Result<String, WasmError> {
    let amount = verus_tx::Amount::from_coins_str(coins).map_err(WasmError::from)?;
    Ok(dto::sats_string(amount))
}
...
pub fn format_coins(satoshis: &str) -> Result<String, WasmError> {
...
mod tests {
    ...
    fn the_amounts_a_float_would_round_are_exact() {
        for (coins, satoshis) in [
            ...
            assert_eq!(parse_coins(coins).unwrap(), satoshis, "coins {coins}");
            assert_eq!(format_coins(satoshis).unwrap(), coins, "sats {satoshis}");
        }
    }

    #[test]
    fn more_precision_than_a_satoshi_has_is_refused() {
        assert!(parse_coins("1.123456789").is_err());
    }
