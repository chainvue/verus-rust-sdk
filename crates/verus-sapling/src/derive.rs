...
pub fn derive_account(
    ...
    coin_type: u32,
    account: u32,
) -> Result<DerivedAccount, SaplingError> {
    ...
    // weaker key.
    if seed.len() < 32 {
        return Err(SaplingError::SeedLength(seed.len()));
    }
    if seed.len() > 252 {
        return Err(SaplingError::SeedLength(seed.len()));
    }
    ...
    if coin_type >= HARDENED_LIMIT {
        return Err(SaplingError::Derivation(format!(
            "coin_type {coin_type} exceeds the hardened index range"
        )));
    }
    if account >= HARDENED_LIMIT {
        return Err(SaplingError::Derivation(format!(
            "account {account} exceeds the hardened index range"
        )));
    }
